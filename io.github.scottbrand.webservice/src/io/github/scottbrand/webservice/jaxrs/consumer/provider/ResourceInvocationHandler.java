...
import io.github.scottbrand.webservice.jaxrs.consumer.RequestConfigurer;
import io.github.scottbrand.webservice.jaxrs.consumer.RequestError;
import io.github.scottbrand.webservice.jaxrs.consumer.RequestException;
...
public class ResourceInvocationHandler implements InvocationHandler
{
	...
	{
		return ConsumerFactory.hasFormAnnotation(method, FormParam.class);
	}
	...
	{
		return ConsumerFactory.hasFormAnnotation(method, FormDataParam.class);
	}
	...
	{
		return Arrays.asList(annotations).stream().filter(a -> a.annotationType() == type).map(a -> type.cast(a)).findFirst().orElse(null);
	}
