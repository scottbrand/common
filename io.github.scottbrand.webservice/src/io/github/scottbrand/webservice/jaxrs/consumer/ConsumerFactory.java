...
import io.github.scottbrand.webservice.jaxrs.consumer.provider.ResourceInvocationHandler;
...
public class ConsumerFactory
{
	...
	public static final Logger log = LoggerFactory.getLogger(ConsumerFactory.class);
	/**
	 * <p>
	 ...
	 *
	 * @param baseUrl
	 *            The server url hosting the specified service.
	 * @param type
	 *            The <code>@Path</code> annotated interface class object.
	 ...
	 *            The server url hosting the specified service.
	 * @param configuration
	 *            The {@link Configuration} to use for building the {@link Client}.
	 * @param type
	 *            The <code>@Path</code> annotated interface class object.
	 * @param customProvider
	 *            An array of <code>@Provider</code> object for de/serialization.
	 * @return a proxy object for the passed in type.
	 */
	...
	public static <T> T createConsumer(String baseUrl, Configuration configuration, Class<T> type)
	{
		...
		return (T) Proxy.newProxyInstance(type.getClassLoader(), new Class<?>[] { type }, createHandler(baseUrl, configuration, path));
	}
	 ...
	 *            The server url hosting the specified service.
	 * @param client
	 *            The {@link Client} to use for sending requests
	 * @param type
	 *            The <code>@Path</code> annotated interface class object.
	 * @param customProvider
	 *            An array of <code>@Provider</code> object for de/serialization.
	...
	public static <T> T createConsumer(String baseUrl, Client client, Class<T> type)
	{
		...
		return (T) Proxy.newProxyInstance(type.getClassLoader(), new Class<?>[] { type }, createHandler(baseUrl, client, path));
	}
	...
	private static boolean hasFormDataParam(Class<?> type)
	{
		...
		{
			if (hasFormAnnotation(method, FormDataParam.class))
			{
				return true;
			}
		}
	...
	private static void ensureTypeIsAnInterface(Class<?> type)
	{
		if (!type.isInterface())
		{
			throw new IllegalArgumentException(type.getName() + " is not an interface. You do not " + "want a dependency to cglib, do you?");
		}
	}
