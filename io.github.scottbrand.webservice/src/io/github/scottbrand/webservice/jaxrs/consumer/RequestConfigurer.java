...
public class RequestConfigurer
{
	...
	{
		return Arrays.asList(annotations).stream().filter(a -> a.annotationType()== PathParam.class).map(a -> ((PathParam)a).value()).findFirst().orElse(null);
	}
	...
	{
		return Arrays.asList(annotations).stream().filter(a -> a.annotationType()== QueryParam.class).map(a -> ((QueryParam)a).value()).findFirst().orElse(null);
	}
	...
	{
		return Arrays.asList(annotations).stream().filter(a -> a.annotationType()== MatrixParam.class).map(a -> ((MatrixParam)a).value()).findFirst().orElse(null);
	}
	...
	{
		return Arrays.asList(annotations).stream().filter(a -> a.annotationType()== HeaderParam.class).map(a -> ((HeaderParam)a).value()).findFirst().orElse(null);
	}
}
